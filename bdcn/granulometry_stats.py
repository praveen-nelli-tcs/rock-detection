...
import cv2
import csv
from rock_detection import curves
import math
from rock_detection import nonmaxsup
import random
import time
from rock_detection import edge_detection_model
import os
from rock_detection import transform_images
...
def filters(image_file):
	...
	# dilate_in = cv2.dilate(median1, kernel4 , iterations = 1 )
	...
	closing1 = cv2.morphologyEx(equ1, cv2.MORPH_CLOSE , kernel1)
...
if __name__ == "__main__":
	image_file = r'C:\Users\2340904\Downloads\bdcn (1)\bdcn\test_images\0006_KOR_KO.211-HC1_20220203_204441_image.png'
	output_res = filters(image_file)
	out_path = r'C:\Users\2340904\Downloads\bdcn (1)\bdcn\test_results\fuse\0006_KOR_KO.211-HC1_20220203_204441_image'
	row, column = output_res.shape
        # filtered_path = os.path.join(result_dir, 'fuse', nm[i], '%s.png' % nm[i])
	cv2.imwrite(out_path+'filtered.png', output_res)

	# Detecting countours in the binary image
	_,edges=cv2.threshold(output_res,50,255,cv2.THRESH_BINARY)
	cnts= cv2.findContours(np.uint8(edges.copy()), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
	cnts = cnts[0]
	...
	cv2.imwrite(out_path+'_result.png', result_image)
